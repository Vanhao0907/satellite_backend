...
from algorithm import (count_time_station_num, data_joint, cal_avail_dmz, save_use_plan,
                       ...
                       iterative_optimization, cal_success_rate, check_crossover_overflow, answer_type_transform, resorted_by_status)
from legacy_config import (ROOT_FOLDER1, ROOT_FOLDER2, OPTIMIZATION, METHOD, ANSWER_TYPE, USE_SA, SA_MAX_TIME,
...
def main():
    ...
    data_QV = read_multiple_csv_files_return_data(ROOT_FOLDER1)
    data_S = read_multiple_csv_files_return_data(ROOT_FOLDER2)
    all_data = data_QV + data_S
    # print('Reading data...',all_data)
    station_name = []
    for qv in data_QV:
        station_name += list(qv.keys())
        # a = 1
    for s in data_S:
        station_name += list(s.keys())
    list_cm_avail = [len(list(e.values())[0]) for e in all_data]
    # list_cm_avail_S = [len(list(e.values())[0]) for e in data_S]
            ...
            if sd > 0:
                ...
                for d0 in range(num_stations):
                    if METHOD == 1:
                        ...
                        d1 = ground_station_start_time_in_one_lap_sorted2_status[-1 - d0]
                    elif METHOD == 3:
                        d1 = ground_station_start_time_in_one_lap_sorted3_status[d0]

                    if ground_station_start_time_in_one_lap[d1] != 1e10:
                        # cm_avail - 当d1地面站可用时的天线数量
                        # d1_cm_end_time - 每个天线的使用结束时间
                        # 输入：每个地面站的天线数量，所有圈数的名字（如'A0007-518'。。。），地面站index，当前圈数index，每一圈结束时间，每个天线的使用结束时间，每个天线上一轮观测的卫星圈数index
                        cm_avail, d1_cm_end_time = cal_avail_dmz(list_cm_avail, keys_line, d1, sp_index,
                                                                 arr_all_end_time,
                                                                 end_time_cm_usage, end_lap_cm_usage)
    ...
    if ANSWER_TYPE == 'TRUE':
        ...
        # 文件名带上时间戳后缀
        file_name = f'{output_dir}/answer_output_{timestamp}.xlsx'
