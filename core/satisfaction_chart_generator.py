...
class SatisfactionChartGenerator:
    ...
    def __init__(self, result_dir, output_dir):
        ...
        logger.info(f"满足度图生成器初始化（原始脚本版）")
    ...
    def generate(self):
        ...
        try:
            ...
            # 导入原始脚本封装的函数（按小时统计版本）
            from satisfaction_original_byhour import generate_satisfaction_chart

            logger.info(f"✓ 成功导入 satisfaction_original_byhour 模块（按小时统计）")
            logger.info(f"调用原始满足度图生成函数...")
            logger.info(f"  源目录: {self.result_dir}")
            logger.info(f"  输出目录: {self.output_dir}")

            # 调用原始脚本函数
            html_content = generate_satisfaction_chart(
