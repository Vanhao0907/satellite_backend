...
def generate_satisfaction_chart_with_figure(source_dir, output_dir):
    ...
    # 第三步: 生成图表 - 测控任务满足度分析图（按小时统计）
    print("\n生成图表: 测控任务满足度分析图（按小时统计）")

    # 按小时统计需求圈次数（所有记录）
    demand_df = final_df.groupby('hour').size().reset_index(name='demand_count')

    # 按小时统计成功圈次数
    success_df = final_df[final_df['success'] == 1]
    success_count = success_df.groupby('hour').size().reset_index(name='success_count')

    # 合并需求数和成功数
    hourly_df = pd.merge(demand_df, success_count, on='hour', how='left').fillna(0)

    # 计算满足率
    hourly_df['satisfaction_rate'] = hourly_df['success_count'] / hourly_df['demand_count'] * 100

    # 计算整体平均满足率
    total_demand = hourly_df['demand_count'].sum()
    total_success = hourly_df['success_count'].sum()
    ...
    # 同时保存文件（保持原有功能）
    chart_path = os.path.join(output_dir, "satisfaction_chart_hourly.html")
    fig1.write_html(chart_path)
    print(f"满足度图已保存: {chart_path}")

    # 同时保存处理后的数据用于验证
    final_df_path = os.path.join(output_dir, "processed_data_hourly.csv")
...
def main():
    ...
    if len(sys.argv) < 3:
        print("用法: python satisfaction_original_byhour.py <source_dir> <output_dir>")
        sys.exit(1)
